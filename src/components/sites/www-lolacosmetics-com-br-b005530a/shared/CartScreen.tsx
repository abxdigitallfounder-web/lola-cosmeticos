...
import { offerDiscount, dailyCouponCode, OFFER_PERCENT } from "./dailyOffer";
...
export default function CartScreen() {
  ...
  return createPortal(<div className="row grid no-gutters lola-basket-original">
    ...
    <aside id="summary" className="col col-12 col-lg-4 basket-summary"><div className="lola-coupon-applied"><span className="lola-coupon-check">✓</span><div><strong>Cupom {dailyCouponCode()} aplicado</strong><small>{OFFER_PERCENT}% de desconto + frete grátis</small></div></div><div id="totla-lola"><div className="row subtotal no-gutters"><div className="col col-12"><span>Subtotal</span><span>{money(subtotal)}</span></div></div><div className="row subtotal no-gutters lola-line-desc"><div className="col col-12"><span>Desconto ({OFFER_PERCENT}%)</span><span>- {money(desconto)}</span></div></div><div className="row subtotal no-gutters lola-line-free"><div className="col col-12"><span>Frete</span><span>GRÁTIS</span></div></div><div className="row row-total no-gutters"><div className="header col col-md-4 col-12">Total:</div><div className="content col col-md-8 col-12"><div className="price"><small className="savings"><span className="instant-price">{money(total)}</span></small><dfn className="condition">em até <span className="parcels">3x</span> de <span className="parcel-value">{money(total / 3)}</span> (sem juros).</dfn></div></div></div><div className="bottom"><div className="wd-checkout-basket-buttons wd-widget"><Link className="bt-checkout btn-big btn-buy" href="/checkout/easy?step=delivery">Finalizar compra</Link><Link className="bt-keep-buying btn-outline" href="/">Continuar comprando</Link></div></div><div id="compra-segura" className="d-flex">Compra 100% segura</div></div></aside>
  </div>, host);
}
