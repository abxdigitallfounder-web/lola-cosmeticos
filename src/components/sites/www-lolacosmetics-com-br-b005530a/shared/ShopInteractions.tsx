...
import { offerDiscount } from "./dailyOffer";
import "./shop.css";
export default function ShopInteractions() {
 ...
 const desconto=offerDiscount(total);
 const aPagar=Math.max(0,total-desconto);
 return <dialog ref={dialog} className="lola-cart" onCancel={()=>setOpen(false)} onClick={e=>{if(e.target===e.currentTarget)setOpen(false)}}><div className="lola-cart-inner"><button className="lola-cart-close" aria-label="Fechar sacola" onClick={()=>setOpen(false)}>×</button><h2>Minha sacola</h2><p>contém {countItems(cart)} itens</p>{cart.length>0&&<OfferBanner compact />}{!cart.length?<div className="lola-empty"><strong>:(</strong><p>Não há produtos na sua sacola. Que tal conferir algumas ofertas?</p></div>:<div className="lola-cart-products">{cart.map(p=><article key={p.id}><img src={p.image} alt={p.name}/><div><h3>{p.name}</h3><strong>{money(p.price)}</strong><div className="lola-quantity"><button aria-label={`Diminuir ${p.name}`} onClick={()=>changeQuantity(p.id,-1)}>−</button><span>{p.quantity}</span><button aria-label={`Aumentar ${p.name}`} onClick={()=>changeQuantity(p.id,1)}>+</button><button className="lola-remove" onClick={()=>removeFromCart(p.id)}>Remover</button></div></div></article>)}</div>}{cart.length>0&&<div className="lola-cart-summary"><div className="lola-cart-line"><span>Subtotal</span><span>{money(total)}</span></div><div className="lola-cart-line lola-cart-desc"><span>Desconto (35%)</span><span>- {money(desconto)}</span></div><div className="lola-cart-line lola-cart-free"><span>Frete</span><span>GRÁTIS</span></div></div>}<div className="lola-cart-total"><span>Total</span><strong>{money(cart.length?aPagar:total)}</strong></div>{cart.length?<Link className="lola-checkout" href="/checkout/easy?step=delivery" onClick={()=>setOpen(false)}>Finalizar Compra</Link>:<button className="lola-checkout" disabled>Finalizar Compra</button>}<button className="lola-continue" onClick={()=>setOpen(false)}>Continuar comprando</button></div></dialog>;
}
